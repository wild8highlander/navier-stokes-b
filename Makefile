...
.PHONY: help install test verify-manifest manifest check-readmes verify-all verify-extended         verify-python verify-julia verify-java         verify-lean verify-coq verify-isabelle verify-agda         verify-rust verify-cpp verify-haskell         docker-build docker-up docker-down         docs docs-serve clean clean-all lint format
...
verify-manifest: ## Check sha256 of every file against MANIFEST.json
        $(PYTHON) .github/scripts/verify_manifest.py

manifest: ## Regenerate MANIFEST.json for the current tree
        $(PYTHON) .github/scripts/regen_manifest.py

check-readmes: ## Verify every directory has a README and none contain Cyrillic
